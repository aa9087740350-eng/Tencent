...
def page_scoring_appendix() -> None:
    ...
    score_source_df = pd.DataFrame(
        [
            ...
            {
                "评分项": "公平感评分",
                "判断依据": "员工对晋升标准、反馈解释、隐性贡献识别、申诉机制和评价一致性的感知评分",
                "权重 / 说明": "不计入总评分；低于 65 会作为员工信任风险信号",
            },
            {
                "评分项": "总评分",
    ...
    st.subheader("发展潜力分计算规则")
    potential_df = pd.DataFrame(
        [
            ["经理评分", "直属经理对员工承担更高职责、复杂任务和岗位成长性的判断", "25%"],
            ["同事评分", "同事对协作能力、影响力和可信赖程度的反馈", "20%"],
            ["考试成绩", "员工对岗位相关知识、制度和专业能力的掌握程度", "15%"],
            ["培训时长", "员工持续学习投入，按最高 100 小时折算为 100 分", "15%"],
            ["管理能力证据", "带团队、项目统筹、人员培养、冲突处理等管理相关证据", "25%"],
            ["高潜员工标记", "被标记为高潜人才时额外加 3 分，最终不超过 100 分", "额外加分"],
        ],
        columns=["计算项", "判断说明", "占比 / 规则"],
    )
    st.dataframe(potential_df, hide_index=True, use_container_width=True)

    st.subheader("公平感评分考虑标准")
    fairness_df = pd.DataFrame(
        [
            ["晋升标准透明度", "员工是否清楚晋升标准、岗位要求和评审流程", "25%"],
            ["反馈解释清晰度", "员工是否理解自己被推荐、复核或暂缓晋升的原因", "25%"],
            ["隐性贡献被看见程度", "跨部门协作、技术攻关、带教新人等贡献是否被纳入评审视野", "20%"],
            ["申诉与补充材料信任度", "员工是否知道并信任补充材料、申诉和人工复核机制", "15%"],
            ["评价一致性感知", "员工是否认为不同部门、不同经理之间的评价标准相对一致", "15%"],
        ],
        columns=["考虑维度", "判断说明", "建议权重"],
    )
    st.dataframe(fairness_df, hide_index=True, use_container_width=True)
    st.caption("当前 Demo 中公平感评分来自模拟员工数据；真实业务中可通过员工问卷、脉搏调研或晋升反馈表采集后计算。")

    st.subheader("岗位匹配分模型")
    selected_role_model = st.selectbox(
        "选择目标晋升岗位",
        ["技术负责人", "团队管理者", "HRBP"],
        key="role_model_selector",
    )
    role_model_map = {
        "技术负责人": [
            ["KPI 完成情况", "25%", "判断候选人是否具备稳定交付和业务结果兑现能力"],
            ["技术攻关", "30%", "判断候选人是否能解决复杂技术问题和关键系统难题"],
            ["项目难度", "20%", "判断候选人承担项目的复杂度、影响范围和技术挑战"],
            ["跨部门协作", "15%", "判断候选人是否能协同产品、运营、客服等团队推动落地"],
            ["知识分享 / 带教新人", "10%", "判断候选人是否能沉淀经验、带动团队技术成长"],
        ],
        "团队管理者": [
            ["团队绩效", "25%", "判断候选人是否能带动团队整体达成目标"],
            ["团队满意度", "25%", "判断候选人的团队氛围建设和人员稳定性"],
            ["人才培养", "20%", "判断候选人是否具备培养新人和梯队建设能力"],
            ["跨部门协作", "15%", "判断候选人是否能协调资源、推动跨团队合作"],
            ["冲突处理 / 组织判断", "15%", "判断候选人是否能处理分歧、做出组织层面的平衡判断"],
        ],
        "HRBP": [
            ["员工满意度", "25%", "判断候选人是否能提升员工体验和组织信任"],
            ["业务支持效果", "25%", "判断候选人是否理解业务并提供有效 HR 解决方案"],
            ["员工关系处理", "20%", "判断候选人是否能处理员工关系、风险事件和沟通问题"],
            ["项目推动能力", "15%", "判断候选人是否能推动组织项目、流程优化和机制落地"],
            ["数据分析能力", "15%", "判断候选人是否能基于数据发现组织问题并支持决策"],
        ],
    }
    role_model_df = pd.DataFrame(
        role_model_map[selected_role_model],
        columns=["能力维度", "权重", "判断重点"],
    )
